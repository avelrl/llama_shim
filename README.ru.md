...
## Тестируемые внешние модели

Список ниже — это рабочий набор моделей для проверок через shim и внешний
upstream. Это не общий бенчмарк и не утверждение о нативном hosted-паритете с
OpenAI. Подробные результаты и интерпретация живут в
...
[100t.xiaomimimo.com](https://100t.xiaomimimo.com/). Прогон дал чистый
результат `28/28` в строгом external tester для shim-owned пути
Responses-over-Chat. Формулировка при этом остается консервативной: результат
подтверждает протестированный chat transport, а не нативный upstream-паритет
`/v1/responses`.

## Документация

- практические руководства: [docs/guides/README.md](docs/guides/README.md)
- инженерные заметки: [docs/engineering/README.md](docs/engineering/README.md)
- заметки по runtime hardening: [docs/engineering/runtime-hardening.md](docs/engineering/runtime-hardening.md)
...
`YAML`. Если рядом лежит локальный `.env`, сервис сначала подхватывает его,
а затем уже дает приоритет реальным переменным процесса. Стартовый шаблон
есть в [.env.example](.env.example).
...
`shimctl` использует тот же `config.yaml` и тот же repo-local `.env`, что и
сам shim, но читает из них только нужный ему subset полей. Для probe-only
override в `.env` используются `SHIMCTL_PROBE_BEARER_TOKEN` и
`SHIMCTL_PROBE_MODEL`. `shimctl probe` запускает ту же calibration-логику по
запросу, без старта HTTP-сервера, печатает live-детали по каждому запросу в
`stderr`, включая полный assistant content для успешных probe, а
структурированный JSON snapshot пишет в `stdout`.
