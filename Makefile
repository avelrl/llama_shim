...
V4_PROVIDER_CONFIG_DOCTOR_FLAGS ?=
MODEL ?=
MODEL_CERT_FLAGS ?=
MODEL_CERT_MODEL_ARG := $(if $(MODEL),-models "$(MODEL)",)
...
build:
	$(TOOL_PREP)
	$(TOOL_ENV) $(GO) build ./cmd/shim ./cmd/shimctl ./cmd/governance-purge-smoke ./cmd/upstream-sse-capture ./cmd/devstack-fixture ./cmd/responses-websocket-smoke ./cmd/codex-eval-runner ./cmd/model-certify
...
devstack-doctor:
	bash ./scripts/devstack-doctor.sh --compose $(DEVSTACK_COMPOSE)

devstack-doctor-advisory:
	DEVSTACK_DOCTOR_REQUIRE_READY=0 bash ./scripts/devstack-doctor.sh --compose $(DEVSTACK_COMPOSE)

preflight-local:
	$(TOOL_PREP)
	$(TOOL_ENV) bash ./scripts/preflight-local.sh
...
upstream-provider-routing-smoke:
...
v4-chat-agent-smoke:
	bash ./scripts/v4-chat-agent-smoke.sh

v4-opencode-smoke:
	bash ./scripts/v4-opencode-smoke.sh

v4-provider-config-doctor:
	$(TOOL_PREP)
	$(TOOL_ENV) $(GO) run ./cmd/shimctl -config $(CONFIG) provider doctor $(V4_PROVIDER_CONFIG_DOCTOR_FLAGS)

v4-provider-matrix-smoke:
	bash ./scripts/v4-provider-matrix-smoke.sh

v4-provider-matrix-curate:
	bash ./scripts/v4-provider-matrix-curate.sh

v4-provider-ops-report:
	bash ./scripts/v4-provider-ops-report.sh

model-certify:
	$(TOOL_PREP)
	$(TOOL_ENV) $(GO) run ./cmd/model-certify $(MODEL_CERT_MODEL_ARG) $(MODEL_CERT_FLAGS)

model-certify-dry-run:
	$(TOOL_PREP)
	$(TOOL_ENV) MODEL_CERT_PHASE=dry-run $(GO) run ./cmd/model-certify $(MODEL_CERT_MODEL_ARG) $(MODEL_CERT_FLAGS)

model-certify-api:
	$(TOOL_PREP)
	$(TOOL_ENV) MODEL_CERT_PHASE=api $(GO) run ./cmd/model-certify $(MODEL_CERT_MODEL_ARG) $(MODEL_CERT_FLAGS)

model-certify-api-extended:
	$(TOOL_PREP)
	$(TOOL_ENV) MODEL_CERT_PHASE=api MODEL_CERT_TESTER_SUITE_CONFIG=configs/suite_responses_extended.yaml MODEL_CERT_TESTER_CAPABILITIES_CONFIG=configs/capabilities_responses_extended.yaml $(GO) run ./cmd/model-certify $(MODEL_CERT_MODEL_ARG) $(MODEL_CERT_FLAGS)

model-certify-codex:
	$(TOOL_PREP)
	$(TOOL_ENV) MODEL_CERT_PHASE=codex $(GO) run ./cmd/model-certify $(MODEL_CERT_MODEL_ARG) $(MODEL_CERT_FLAGS)

codex-config:
	$(TOOL_PREP)
	$(TOOL_ENV) $(GO) run ./cmd/shimctl -config $(CONFIG) codex config

codex-config-doctor:
	$(TOOL_PREP)
	$(TOOL_ENV) $(GO) run ./cmd/shimctl -config $(CONFIG) codex doctor
...
codex-eval-curate:
	$(TOOL_PREP)
	$(TOOL_ENV) bash ./scripts/codex-eval-curate.sh
