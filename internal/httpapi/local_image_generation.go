...
func localImageGenerationDisabledError() error {
	return domain.NewValidationError("tools", "shim-local image_generation runtime is disabled; set responses.image_generation.backend to responses, comfyui, or fixture, or use responses.mode=prefer_upstream")
}
...
func (h *responseHandler) createLocalImageGenerationResponse(ctx context.Context, request CreateResponseRequest, requestJSON string, rawFields map[string]json.RawMessage) (domain.Response, []domain.ResponseReplayArtifact, error) {
	...
	upstreamBody, _, err := buildUpstreamResponsesBody(rawFields, prepared.ContextItems, prepared.NormalizedInput, prepared.ToolCallRefs, h.customToolsMode, h.codexCompatibilityEnabled)
	if err != nil {
		return domain.Response{}, nil, err
	}
