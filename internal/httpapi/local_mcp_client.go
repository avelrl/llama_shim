...
const localMCPProtocolVersion = "2024-11-05"
...
func (c *localMCPClient) ListTools(ctx context.Context, serverURL string) ([]localMCPTool, error) {
	session, err := c.openLegacySession(ctx, serverURL)
	if err != nil {
		return nil, err
	}
	defer session.Close()
...
func (c *localMCPClient) CallTool(ctx context.Context, serverURL string, name string, arguments json.RawMessage) (localMCPToolCallResult, error) {
	session, err := c.openLegacySession(ctx, serverURL)
	if err != nil {
...
func (c *localMCPClient) openLegacySession(ctx context.Context, serverURL string) (*localMCPLegacySession, error) {
...
func (s *localMCPLegacySession) request(ctx context.Context, method string, params any) (json.RawMessage, error) {
	...
	for {
		...
		if !localMCPJSONRPCIDMatches(response.ID, id) {
			continue
		}
		if response.Error != nil {
			...
			return nil, fmt.Errorf("MCP %s failed: %s", method, message)
		}
