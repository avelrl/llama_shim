...
const (
	...
	codexCompatibilityHint = "Codex compatibility rules for this environment: use exec_command in the sandbox by default for normal workspace reads, writes, and local commands. Only request sandbox_permissions=require_escalated for real sandbox limits such as network access, GUI apps, destructive actions, or writes outside the workspace; if approval policy is never, do not request escalation. For file edits, use the apply_patch tool directly for existing files and use exec_command mainly for reads, builds, tests, and other shell tasks. Never invoke apply_patch through exec_command. The exec_command.cmd field must be a single shell string, not an argv array. Avoid rewriting an existing file with heredocs, `cat > file`, `echo ... | base64 -d`, or similar whole-file shell writes when a targeted patch is possible. " +
		applyPatchRawInputFormatHint +
		" When using exec_command, prefer structured arguments such as workdir instead of `cd ... &&`. For Go tests that fail because the default Go build cache is outside writable roots, rerun with GOCACHE and GOTMPDIR under /tmp, then continue diagnosing the actual test failure. For test or lint commands, set a generous yield_time_ms so the command can finish without extra polling. Never write pseudo-tool calls as assistant text, including forms like `<tool call: exec_command(...)>`, `[Tool call: apply_patch]`, `[shell_command]`, `<tool_call>`, `<function_call>`, `<function name=...>`, `<tool_code_call>`, `<tool_code_exec>`, `<tool_code_interpreter>`, `<chatcmpl-tool>`, `<function.chatcmpl.tool>`, `<tools>`, `<read_file>`, `<bash>`, `<patch>`, `<apply_patch>`, `<command>`, `<command-message>`, `<command-name>`, `<command-output>`, `<command-arg>`, `<prelude>`, `<|mask_start|>`, `<|mask_end|>`, fenced command/apply_patch blocks, or `<function_call_output>`; use structured tool calls instead. Do not describe a patch as a code block when the task requires editing files; call apply_patch or exec_command as appropriate. After the final tool result, always send a brief assistant message to the user and do not end the turn with only reasoning or plan updates. Do not describe future tool actions after tools have already run; if the user requested an exact final marker or success word after verification, include that marker exactly in the final message."
)
...
const (
	...
	codexApplyPatchToolHint  = "Codex rule: use this tool directly for existing-file edits instead of invoking apply_patch through exec_command or shell wrappers. " + applyPatchRawInputFormatHint
)
