...
| `responses.mode`, `responses.upstream_transport`, `llama.providers` / `provider/model` routing, local backends, `/debug/capabilities`, `/healthz`, `/readyz`, `/metrics` | No | No | Shim-owned operational and routing surface; `llama.providers` / `provider/model` is implemented as a V3 shim-owned extension for configured `/v1/responses`, Responses WebSocket `response.create`, `/v1/responses/input_tokens`, `/v1/responses/compact`, `/v1/chat/completions`, `/v1/models`, readiness, and capabilities paths, not as hosted OpenAI parity |
