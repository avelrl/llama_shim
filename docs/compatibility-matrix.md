...
- Official native local `shell` and `apply_patch` are implemented as
  ...
  still exercises the bridge path, including the repo-owned coding-task smoke,
  rather than native tool declarations from the CLI.
