...
| local `image_generation` | Broad subset | Keep separate image-backend contract explicit | Current subset supports the Responses-compatible image backend and the deterministic fixture backend; it is docs-aligned, not exact hosted planner parity |
