...
| Semantic SSE and Responses WebSocket `response.create` | Yes | Yes | Broad subset; exact hosted tool-specific choreography remains outside the broad claim unless backed by docs or fixtures; configured provider routing validates model-bearing WebSocket create and warmup messages |
...
| `responses.mode`, `responses.upstream_transport`, `llama.providers` / `provider/model` routing, local backends, `/debug/capabilities`, `/debug/traces`, `/healthz`, `/readyz`, `/metrics` | No | No | Shim-owned operational and routing surface; `llama.providers` / `provider/model` is implemented as a V3 shim-owned extension for configured `/v1/responses`, Responses WebSocket `response.create`, `/v1/responses/input_tokens`, `/v1/responses/compact`, `/v1/chat/completions`, `/v1/models`, readiness, capabilities, plugin-contract metadata, and debug-trace paths, not as hosted OpenAI parity |
...
| create-stream and retrieve-stream | Broad subset | Keep core SSE stable; do not overclaim exact hosted choreography | Generic replay is accepted where docs or fixtures do not lock down tool-specific event families. Shim-local text create-stream can buffer model output before the first text delta for raw tool-markup repair, so token-by-token hosted streaming fidelity is not claimed. |
| Responses WebSocket mode | Broad subset | Keep the local transport boundary explicit | `GET /v1/responses` now accepts WebSocket upgrades, processes sequential `response.create` messages for the current shim-local Responses subset, validates configured provider aliases for generated and `generate:false` turns, and emits existing Responses streaming events as JSON text frames. `/debug/capabilities` advertises the local subset. No multiplexing, no upstream WebSocket proxying, and no exact hosted close-code/quota/cache parity claimed; those remain in [v5-scope.md](v5-scope.md). |
...
| `responses.upstream_transport=chat_completions` | Broad subset | Keep as a shim-owned chat-only upstream bridge | The shim synthesizes the Responses surface locally and calls upstream `/v1/chat/completions` only for model generation. Local storage, retrieve, `previous_response_id`, `conversation`, `input_items`, generic SSE/replay, and `text.format` to Chat `response_format` mapping are in scope; native hosted Responses planner/tool choreography is not claimed. |
...
| `/v1/responses/input_tokens` | Broad subset | Keep “local deterministic estimate” wording explicit | V2 does not require exact upstream tokenization parity; when provider routing is enabled, model-bearing upstream proxy calls route by public `provider/model`, while model-less supported local requests stay local |
| `/v1/responses/compact` | Broad subset | Keep standalone compaction subset explicit | V3 adds `model_assisted_text` canonical next-window output with retained items plus a shim-owned opaque compaction item; exact hosted encrypted compaction state is not claimed; model-bearing upstream proxy calls use configured provider routing, and model-less unsupported compact requests fail locally instead of falling through to a hidden provider |
...
| local `image_generation` | Broad subset | Keep separate image-backend contract explicit | Current subset supports the Responses-compatible image backend, generic ComfyUI text-to-image workflows, and the deterministic fixture backend; it is docs-aligned, not exact hosted planner parity |
| local `computer` | Broad subset | Keep external-loop contract explicit | Current subset is screenshot-first, allow-lists documented action types, preserves `computer_call_output` screenshot detail on Responses items, maps `original` to `high` for the Chat-backed planner projection, and intentionally stays generic on replay |
| local `code_interpreter` | Broad subset | Keep dev-only/local boundary explicit | Current contract is useful but not hosted-equivalent; in Postgres mode session/container-file state remains per-instance SQLite sidecar state, while mirrored `/v1/files` follow the configured file store |
...
| local `computer` | local subset when backend is configured | proxy first | local subset or explicit disabled-runtime error | `prefer_upstream` stays raw-proxy; current local subset is screenshot-first external-loop planning with allow-listed action types |
...
| hard-delete/governance purge | Implemented shim-owned operator subset | Do not expose as hidden behavior inside OpenAI-shaped routes | `shimctl governance purge` supports the `all_local_state` operator scope with dry-run by default, explicit apply confirmation, bounded table batches, audit JSON, SQLite coverage, and Postgres-plus-sidecar coverage. Current OpenAI-shaped resource deletes remain scoped to the addressed resource. Tenant/project purge, redaction policy, legal hold, approval workflow, upstream delete propagation, and backup/PITR deletion guarantees remain out of scope; see [v3-hard-delete-governance.md](v3-hard-delete-governance.md). |
