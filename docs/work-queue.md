...
| local GPU batch (`qwen3_6-35b-a3b`, `glm4_7-flash`, `qwen35-27b-opus-reasoning`, `qwen3-coder-next`, `qwen3-30b-instruct`, `qwen3-next-instruct`, `gpt-oss-20b`, `gpt-oss-120b`) | Configured candidates; GPT-OSS Codex promotion blocked | Aliases are configured under the `gpu` provider with conservative Codex metadata unless a prior tested model already established a larger context. GPT-OSS candidates start at 32768 tokens and use Chat upstream compatibility cleanup for `developer` role and default thinking. Evidence on May 19, 2026: `gpt-oss-20b` is API/Responses usable but fails Codex baseline 6/11 and has native Chat stream issues; `gpt-oss-120b` is API/Chat/Responses ready but fails Codex baseline 7/11, then passed Chat-agent/OpenCode smokes at `.tmp/v4-chat-agent-smoke/gpu-gpt-oss-120b_20260520T230902Z` and `.tmp/v4-opencode-smoke/gpu-gpt-oss-120b_20260520T231135Z`. `gpu/qwen3-coder-next` maps to the local runtime id `qwen3-coder-next` for Qwen3 Coder Next and passed API certification `.tmp/model-certification/cert-20260520T221144Z`, plus Chat-agent/OpenCode evidence at `.tmp/v4-chat-agent-smoke/gpu-qwen3-coder-next_20260520T210116Z` and `.tmp/v4-opencode-smoke/gpu-qwen3-coder-next_20260520T210229Z`. The certification fix-candidate is from expected negative-shape `400` checks, not a failed API gate. Codex baseline `.tmp/model-certification/cert-20260520T221225Z` failed 10/11 on `plan_doc`, with upstream `context canceled`/`502 transport_error` noise and raw tool markup/final-marker misses. `gpu/qwen3-next-instruct` passed Chat-agent at `.tmp/v4-chat-agent-smoke/gpu-qwen3-next-instruct_20260520T213404Z`, but OpenCode failed at `.tmp/v4-opencode-smoke/gpu-qwen3-next-instruct_20260520T213441Z` because `calc.go` was not patched. `gpu/qwen3-30b-instruct` now has Chat-agent and OpenCode evidence: `.tmp/v4-chat-agent-smoke/gpu-qwen3-30b-instruct_20260520T134315Z` and `.tmp/v4-opencode-smoke/gpu-qwen3-30b-instruct_20260520T192306Z`. `qwen35-27b-opus-reasoning` is newly configured and still evidence-pending. | Keep GPT-OSS as supervised API/Chat/OpenCode candidates, not unattended Codex gates. Treat `gpu/qwen3-coder-next` as Chat/OpenCode/API green but Codex-blocked until baseline is clean. Treat `gpu/qwen3-next-instruct` as mixed until real-client evidence improves. |
