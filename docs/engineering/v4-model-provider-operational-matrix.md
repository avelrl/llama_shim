...
| `gpu/qwen3-coder-30b` | Fast local candidate | Not promoted to provider matrix gate yet | Certification evidence: recent baseline passes; expanded `cert-20260519T144415Z` failed 17/18 on `multi_file`; bench-lite `cert-20260519T162448Z` is 19/20 with repeated `patch_after_context` miss. `command_recovery` stays fixed. Chat-agent smoke passed in `.tmp/v4-chat-agent-smoke/gpu-qwen3-coder-30b_20260520T194113Z`; OpenCode smoke passed in `.tmp/v4-opencode-smoke/gpu-qwen3-coder-30b_20260520T194141Z`. | `32768` context, `shell_command`, `apply_patch=freeform` | Local llama.cpp-compatible GPU alias resolves to upstream runtime id `coder30b`. Chat cleanup downgrades JSON Schema to JSON mode plus schema instruction for sampler compatibility. Raw tool markup now surfaces as classified `502 malformed_backend_response`, not hidden `500`. | Usable as a supervised local Chat/OpenCode coding assistant. Do not use as unattended release gate until bench-lite is clean or an operator explicitly accepts the repeated partial-edit risk. |
| `gpu/qwen3-coder30b-q5km` | Stronger local Codex and Chat-agent candidate | Not promoted to provider matrix gate yet | Model certification `cert-20260519T220117Z` passed all focused Codex profiles: baseline 11/11, expanded 18/18, bench-lite 20/20, for 49/49 final tasks. Verdict is `codex_retry_dependent`: 6 failed attempts were recovered by retries. Chat-agent smokes passed in `.tmp/v4-chat-agent-smoke/gpu-qwen3-coder30b-q5km_20260520T134047Z` and `.tmp/v4-chat-agent-smoke/gpu-qwen3-coder30b-q5km_20260520T175840Z`; OpenCode smoke passed in `.tmp/v4-opencode-smoke/gpu-qwen3-coder30b-q5km_20260520T180029Z`. | `32768` context, `shell_command`, `apply_patch=freeform` | Local GPU alias resolves to upstream runtime id `coder30b-q5km`. API certification `cert-20260520T180503Z` failed at external tester `chat.basic`: expected exact `OK`, got `pong`. Invalid-shape `400` traces were expected validation behavior, not shim regression. Retry evidence includes raw-tool-markup repair, malformed backend responses, one transport failure, and one timeout. Chat tool compatibility now repairs raw pseudo-tool markup in assistant content before it reaches Chat-first clients. | Best current local Codex candidate and a green Chat-first coding-agent candidate. Usable for supervised coding and larger local diagnostics; do not call it strict-clean or unattended release-gate clean until retry rate is lower. Treat the `pong` result as model exactness weakness, not a transport blocker. |
...
| `gpu/glm47-flash-opus-reasoning` | Local reasoning/API candidate | Not promoted to provider matrix gate yet | API certification `cert-20260520T115344Z` was 27/28: only native Chat streaming returned `H` instead of `HELLO`, while Responses stream, structured output, tools, custom tools, `store=true`, conversations, and `previous_response_id` paths passed. Codex baseline `cert-20260520T120358Z` failed 8/11: `basic_patch` checker diff plus `bugfix_go`/`bugfix_mixed` malformed `apply_patch` responses. Chat-agent smoke passed in `.tmp/v4-chat-agent-smoke/gpu-glm47-flash-opus-reasoning_20260520T194326Z`; OpenCode smoke passed in `.tmp/v4-opencode-smoke/gpu-glm47-flash-opus-reasoning_20260520T194446Z`. | `32768` context, `shell_command`, `apply_patch=freeform` | Local GPU alias resolves to upstream runtime id `glm47-flash-opus-reasoning`. API behavior is promising for reasoning/content cleanup diagnostics, but Codex failures show patch-contract instability rather than broad transport failure. | Keep experimental. Useful for API/chat diagnostics and reasoning-content stress; do not use as a Codex gate until baseline patch/tool-call behavior is clean. |
| `gpu/qwen3-30b-instruct` | Local Chat-agent baseline | Not promoted to provider matrix gate yet | Chat-agent smoke passed in `.tmp/v4-chat-agent-smoke/gpu-qwen3-30b-instruct_20260520T134315Z`, covering streaming, file tools, Go bugfix with real `go test ./...`, and multi-file edit. OpenCode smoke passed in `.tmp/v4-opencode-smoke/gpu-qwen3-30b-instruct_20260520T192306Z`. | `32768` context, `shell_command`, `apply_patch=freeform` | Local GPU alias resolves to upstream runtime id `qwen3-30b-instruct`. Keep this as a Chat-first coding-agent baseline; Codex evidence should be recorded separately before using it as a Codex gate. | Good local Chat-first candidate. Use for OpenCode/Aider-style smoke before spending time on heavier Codex profiles. |
| `gpu/qwen3-coder-next` | Local Chat/API candidate, Codex baseline blocked | Not promoted to provider matrix gate yet | API certification `.tmp/model-certification/cert-20260520T221144Z` passed with verdict `api_compat_passed`. Chat-agent smoke passed in `.tmp/v4-chat-agent-smoke/gpu-qwen3-coder-next_20260520T210116Z`, covering streaming, file tools, Go bugfix with real `go test ./...`, and multi-file edit. OpenCode smoke passed in `.tmp/v4-opencode-smoke/gpu-qwen3-coder-next_20260520T210229Z`. Codex baseline `.tmp/model-certification/cert-20260520T221225Z` failed 10/11: final failure was `plan_doc`, with transient `bugfix_go` timeout and `command_recovery` checker miss recovered by retry. | `32768` context, `shell_command`, `apply_patch=freeform` | Local GPU alias resolves to upstream runtime id `qwen3-coder-next` for Qwen3 Coder Next. The API certification fix-candidate came from expected negative-shape `400` checks, not a failed API gate. Codex failure included upstream `context canceled`/`502 transport_error` noise plus `plan_doc` raw `<|tool_call>`/`<|tool_calls_section>` leakage and missing `PLANNED` final marker. | Promising Chat/OpenCode/API candidate, but do not use as Codex gate. Re-run baseline only after upstream stability improves or a targeted raw-markup/plan-doc fix is justified. |
| `gpu/qwen3-next-instruct` | Local Chat-agent candidate | Not promoted to provider matrix gate yet | Chat-agent smoke passed in `.tmp/v4-chat-agent-smoke/gpu-qwen3-next-instruct_20260520T213404Z`. OpenCode smoke `.tmp/v4-opencode-smoke/gpu-qwen3-next-instruct_20260520T213441Z` failed because the client did not patch `calc.go`. | `32768` context, `shell_command`, `apply_patch=freeform` | Local GPU alias resolves to upstream runtime id `qwen3-next-instruct`. Codex certification evidence is pending. | Keep as mixed: shim-owned Chat-agent path works, but real-client OpenCode is not green. Do not promote until OpenCode or Codex evidence improves. |
...
| `gpu/gpt-oss-120b` | Larger local API/Chat candidate, Codex experimental | API certification `cert-20260519T201957Z` passed: Chat and Responses both ready, including Chat stream. Chat-agent smoke passed in `.tmp/v4-chat-agent-smoke/gpu-gpt-oss-120b_20260520T230902Z`; OpenCode smoke passed in `.tmp/v4-opencode-smoke/gpu-gpt-oss-120b_20260520T231135Z`. | Codex baseline `cert-20260519T202427Z` failed 7/11: `bugfix_go`, `bugfix_mixed`, `command_recovery`, and `multi_file` remained red. | `32768` context, `shell_command`, `apply_patch=freeform` | Better than 20B on API and Codex baseline, but still has transport/context-canceled noise and incomplete edit/tool-loop behavior. Failures include no file change, missing final markers, and files described as updated before they actually match. | Keep as a supervised Chat/OpenCode assistant and model-quality comparison point. Do not promote as an unattended Codex gate until baseline is clean. |
