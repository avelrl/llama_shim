...
| `gpu/qwen3-coder-next` | Local Chat-agent candidate | Not promoted to provider matrix gate yet | Chat-agent smoke passed in `.tmp/v4-chat-agent-smoke/gpu-qwen3-coder-next_20260520T210116Z`, covering streaming, file tools, Go bugfix with real `go test ./...`, and multi-file edit. OpenCode smoke passed in `.tmp/v4-opencode-smoke/gpu-qwen3-coder-next_20260520T210229Z`. | `32768` context, `shell_command`, `apply_patch=freeform` | Local GPU alias resolves to upstream runtime id `qwen3-coder-next` for Qwen3 Coder Next. Codex certification evidence is pending. | Promising Chat/OpenCode candidate. Run Codex baseline before considering it for broader coding-agent gate use. |
