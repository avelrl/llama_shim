...
The shim-supported Lark subset is intentionally narrower than full Lark but is
kept aligned with OpenAI custom-tool guidance: built-in `%import common.*`
imports are allowed, including grouped imports such as
`%import common (CNAME, NEWLINE)`, while non-`common` imports, `%declare`,
lookaround regexes, and lazy regex/grammar quantifiers are rejected before
adapter dispatch. Codex freeform tools currently exercise this path through the
`apply_patch`, `js_repl`, and code-mode grammars in the local Codex source.
